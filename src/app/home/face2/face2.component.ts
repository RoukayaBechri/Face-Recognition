...
export class Face2Component implements OnInit {
  ...
  Osadness; Ohappy; Oneutral; Ofear; Oanger; Osurprise;
...
this.Ohappy= Math.floor(this.personAge[i].faceAttributes.emotion.happiness*100);
this.Ofear= Math.floor(this.personAge[i].faceAttributes.emotion.fear*100);
this.Osurprise= Math.floor(this.personAge[i].faceAttributes.emotion.surprise*100);
this.Oanger= Math.floor(this.personAge[i].faceAttributes.emotion.anger*100);
this.Oneutral= Math.floor(this.personAge[i].faceAttributes.emotion.neutral*100);
this.Osadness= Math.floor(this.personAge[i].faceAttributes.emotion.sadness*100);
        anger = anger + this.personAge[i].faceAttributes.emotion.anger;
        pAnger= Math.floor((this.personAge[i].faceAttributes.emotion.anger)*100);

        fear = fear + (this.personAge[i].faceAttributes.emotion.fear);
        pFear= Math.floor((this.personAge[i].faceAttributes.emotion.fear)*100);

        hapiness = hapiness + (this.personAge[i].faceAttributes.emotion.happiness);
        pHapiness= Math.floor((this.personAge[i].faceAttributes.emotion.happiness)*100);

        neutral = neutral + (this.personAge[i].faceAttributes.emotion.neutral);
        pNeutral=Math.floor((this.personAge[i].faceAttributes.emotion.neutral)*100);

        sadness = sadness + (this.personAge[i].faceAttributes.emotion.sadness);
        pSadeness= Math.floor((this.personAge[i].faceAttributes.emotion.sadness)*100);

        surprise = surprise + (this.personAge[i].faceAttributes.emotion.surprise);
        pSurprise= Math.floor((this.personAge[i].faceAttributes.emotion.surprise)*100);
      ...
      console.log('male:' + this.personMale);
        ...
        this.percentSurprise = Math.floor((surprise / this.personNumber) * 100);
        console.log(' surprise:' + surprise);
        console.log('numer person:' + this.personNumber);
        console.log('pourcentage surprise:' + this.percentSurprise);
        this.percentAnger = Math.floor((anger / this.personNumber) * 100);
        console.log('pourcentage anger: an' + this.percentAnger);
        this.percentFear = Math.floor((fear / this.personNumber) * 100);
        console.log('pourcentage fear;' + this.percentFear);
        this.percentSadeness = Math.floor((sadness / this.personNumber) * 100);
        console.log('pourcentage sadness:' + this.percentSadeness);
        this.percentNeutral = Math.floor((neutral / this.personNumber) * 100);
        console.log('pourcentage neutral:' + this.percentNeutral);
        this.percentHapiness = Math.floor((hapiness / this.personNumber) * 100);
